...
// 1 초	256 MB	46835	24871	22208	53.992%
// 문제
...
for(var i = 0; i < input.length; i++){
    input[i] = input[i].split(" ").map(item => Number(item));
}

let array = new Array(input[0][0]).fill(0).map((_,i) => i + 1);

let answer = [];
for(var i = 1; i <= input[0][1]; i ++){
    var startIndex = input[i][0] -1;
    var endIndex = input[i][1];
    answer = array.slice(startIndex, endIndex).reverse();
    array.splice(startIndex, answer.length, ...answer);
}
