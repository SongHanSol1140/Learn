...
// 1 초	256 MB	56030	29600	26422	53.530%
// 문제
...
var throwChance = Number(input[0].split(" ")[1]);
var basketState = new Array(basketCount).fill(0);

for(var i = 1; i <= throwChance; i++){
    var startNum = input[i].split(" ")[0];
    var endNum = input[i].split(" ")[1];
    var ballPoint = input[i].split(" ")[2];
    startNum--;
    endNum--;
    for(var j = startNum; j <= endNum; j++){
        basketState[j] = ballPoint;
    }
}
